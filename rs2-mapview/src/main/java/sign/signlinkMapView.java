...
public class signlinkMapView implements Runnable {
	...
	public static ClientMapView mainapp;
	...
	public static void startpriv(@OriginalArg(0) InetAddress address) {
		...
		@Pc(33) Thread thread = new Thread(new signlinkMapView());
	...
	public static void reporterror(@OriginalArg(0) String message) {
		...
		try {
			...
			@Pc(46) DataInputStream stream = openurl("reporterror" + signlinkMapView.clientversion + ".cgi?error=" + errorname + " " + message);
			stream.readLine();
			stream.close();
		...
		while (threadliveid == threadId) {
			if (socketreq != 0) {
				...
				if (midiplay) {
					...
					if (!isAndroid)
						ClientMapView.clientMapView.post(new MidiPlay(midi));
					midiplay = false;
				}
