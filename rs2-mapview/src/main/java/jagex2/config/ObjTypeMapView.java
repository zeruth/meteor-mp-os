...
public class ObjTypeMapView {
	...
	private static ObjTypeMapView[] cache;
	...
	public static void unpack(@OriginalArg(0) Jagfile config) {
		...
		cache = new ObjTypeMapView[10];
		for (@Pc(51) int id = 0; id < 10; id++) {
			cache[id] = new ObjTypeMapView();
		}
	}
	...
	public static ObjTypeMapView get(@OriginalArg(0) int id) {
		...
		@Pc(27) ObjTypeMapView obj = cache[cachePos];
		dat.pos = offsets[id];
		obj.index = id;
		obj.reset();
		obj.decode(dat);
	...
	public static Pix32MapView getIcon(@OriginalArg(0) int id, @OriginalArg(2) int count) {
		...
		@Pc(28) ObjTypeMapView obj = get(id);
	...
	public void toCertificate() {
		@Pc(3) ObjTypeMapView template = get(this.certtemplate);
		...
		@Pc(55) ObjTypeMapView link = get(this.certlink);
		this.name = link.name;
