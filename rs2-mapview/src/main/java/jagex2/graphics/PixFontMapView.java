...
public class PixFontMapView extends Pix2DMapView {
	...
	public PixFontMapView(@OriginalArg(0) Jagfile title, @OriginalArg(1) String font) {
	...
	private void drawChar(@OriginalArg(0) byte[] data, @OriginalArg(1) int x, @OriginalArg(2) int y, @OriginalArg(3) int w, @OriginalArg(4) int h, @OriginalArg(5) int rgb) {
		@Pc(5) int dstOff = x + y * Pix2DMapView.width2d;
		@Pc(9) int dstStep = Pix2DMapView.width2d - w;
		...
		if (y + h >= Pix2DMapView.boundBottom) {
			h -= y + h + 1 - Pix2DMapView.boundBottom;
		}
		...
		if (x + w >= Pix2DMapView.boundRight) {
			int cutoff = x + w + 1 - Pix2DMapView.boundRight;
			w -= cutoff;
			srcStep += cutoff;
			dstStep += cutoff;
		}
		...
		if (w > 0 && h > 0) {
			this.drawMask(w, h, data, srcOff, srcStep, Pix2DMapView.data, dstOff, dstStep, rgb);
		}
	}
	...
	private void drawCharAlpha(@OriginalArg(2) int x, @OriginalArg(5) int y, @OriginalArg(7) int w, @OriginalArg(3) int h, @OriginalArg(4) int rgb, @OriginalArg(6) int alpha, @OriginalArg(0) byte[] mask) {
		@Pc(10) int dstOff = x + y * Pix2DMapView.width2d;
		@Pc(14) int dstStep = Pix2DMapView.width2d - w;
		...
		if (w > 0 && h > 0) {
			this.drawMaskAlpha(w, h, Pix2DMapView.data, dstOff, dstStep, mask, srcOff, srcStep, rgb, alpha);
		}
	}
